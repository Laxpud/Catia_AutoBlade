# CATIA AutoBlade 项目指南

本文是 AI 和维护者的工作入口，只提供文档路由、执行流程和容易造成严重回归的项目边界。公开能力与使用方法以 `README.md` 为准，技术细节以 `docs/` 为准。

## 项目边界

CATIA AutoBlade 是通过 CATIA V5 COM Automation 生成叶片模型的 Windows Python CLI。当前交付目标是受控团队使用的内部 `v0.x` preview wheel；公共 PyPI、独立 EXE、`sweep` 和第二 CAD 后端都不是已有能力。
...
- 开发中先运行与改动对应的测试；非简单改动交付前运行 `pwsh -File scripts/check.ps1`。
- 默认 pytest 使用 fake/mock COM。真实 CATIA 冒烟测试只能显式人工执行，并按 `docs/testing.md` 记录环境、输入、CATPart/STEP、特征树和残留 `CNEXT` 进程。

## 不可破坏的边界

- `catia_autoblade.core` 不得导入 `pythoncom`、`win32com` 或持有 CATIA COM 对象；后端不可用时返回项目定义的后端异常。
...
- `create` 只生成一个任务；`batch` 按选中的截面定义生成任务且不做隐式笛卡尔积；参数组合只属于未来显式的 `sweep`。
- 不得绕过 `tests/conftest.py` 的真实 COM 防护。修改输入拓扑、后缘判定或 CATIA 几何编排时，必须补充对应回归。
- 唯一版本源是 `src/catia_autoblade/__init__.py` 的 `__version__`；不要在其他文件建立独立版本值。
