...
- 当前安装仍依赖源码 checkout；仓库尚无发布标签，也没有受支持的 wheel 分发渠道。
- 当前目标是交付面向受控工程团队的内部 `v0.x` preview wheel，不把公共 PyPI、独立 EXE、`sweep` 或第二 CAD 后端纳入首版阻塞项。
...
- [ ] 固定 Hatchling wheel 与 sdist 文件清单，避免构建结果受本地临时文件影响。
...
- [ ] 从候选 wheel 创建全新环境并完成非 editable 安装冒烟。
- [ ] `autoblade init` 创建的工作区可完成输入发现、配置读取和代表模型预检。
- [ ] 候选 wheel 在已记录的支持基线上完成真实 CATIA 建模、CATPart/STEP 导出和进程清理。
