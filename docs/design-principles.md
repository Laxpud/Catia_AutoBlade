...
| `sweep` | 显式组合多个设计变量 | N × M × … 个 `BladeBuildJob` | 规划中，尚无命令 |

`batch` 的“多个”来自多个截面定义，不来自目录中碰巧存在的多个翼型。多个六列截面模板可以统一绑定一个显式翼型；多个翼型与多个模板的组合只能由未来 `sweep` 明确表达。这样新增输入文件不会静默改变已有批处理的任务数。
...
Planner 在 CATIA 会话创建前解析所有输入，包括唯一翼型、逐截面引用、点序、后缘拓扑、输出名称和同批次目标冲突。Executor 不根据目录内容推断模型，也不知道任务来自 `create`、`batch` 还是未来 `sweep`。
...
- 显式子命令是脚本和可复现调用的稳定接口。`create --interactive` 与 `batch --interactive` 是直接进入对应向导的兼容入口。
...
`autoblade-create` 与 `autoblade-batch` 保留为兼容入口，并与对应子命令共享参数和退出码。Python 层旧的批处理入口也不再接受多个翼型生成隐式组合；调用方需要等待 `sweep` 或显式逐次调用。
